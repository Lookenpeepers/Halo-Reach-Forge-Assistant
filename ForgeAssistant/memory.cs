    ...
    public class Mem
    {
        ...
        public bool OpenProcess(int pid)
        {
            ...
            try
            {
                ...
                pHandle = OpenProcess(AccessFlags.All, true, pid);
        ...
        public Task<IEnumerable<long>> AoBScan(long start, long end, string search, bool readable, bool writable, bool executable, string file = "")
        {
            return Task.Run(() =>
            {
                ...
                //Main.progressBar2.Maximum = 0;
                //Main.progressBar2.Value = 1;
                ...
                //int prog = 0;
                //Main.progressBar2.Maximum = memRegionList.Count;
                Parallel.ForEach(memRegionList,
                                 (item, parallelLoopState, index) =>
                                 {
                                     //prog += 1;
                                     //Main.backgroundWorker2.ReportProgress(prog);
                                     
                                     long[] compareResults = CompareScan(item, aobPattern, mask);
                                     //Main.progressBar2.Maximum = ((int)compareResults.Count() * memRegionList.Count);
                                     foreach (long result in compareResults)
                                         bagResult.Add(result); //Main.progressBar2.Value += 1;
                                 });
        ...
        private long[] CompareScan(MemoryRegionResult item, byte[] aobPattern, byte[] mask)
        {
            ...
            IntPtr buffer = Marshal.AllocHGlobal((IntPtr)item.RegionSize);
